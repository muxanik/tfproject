<script src="<?=SITE_TEMPLATE_PATH?>/js/jquery-calx-2.2.6.min.js"></script>
<script src="<?=SITE_TEMPLATE_PATH?>/js/odometer.js"></script>
<?$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH . '/js/imageMapResizer.min.js');?>
<script>
...
<?$iq=1;?>
		<div class="col-2-div uslugi" style="float:right;">
			...
			<?foreach($arResult["PROPERTIES"]["PRICE"]["VALUE"] as $key => $arImage):?>
				...
				<?if($arResult["PROPERTIES"]["MOTODISK"]["VALUE"] == "Да"):?>			
				<div id="tabs-<?=$key?>"><span style="color:#000;font-size:12px;">за 1 диск:</span> <?=$priceX1?> ₽ <input hidden data-cell="L<?=$iq++;?>" value="<?=$priceX1?>"><br /><span style="color:#000;font-size:12px;">за 2 диска:</span> <?=$priceX2?> ₽</div>			
				<?else:?>
                ...
                <?else:?>
				<div id="tabs-<?=$key?>"><span style="color:#000;font-size:12px;">за 1 диск:</span> <?=$priceX1?> ₽ <input hidden data-cell="L<?=$iq++;?>" value="<?=$priceX1?>"><br /><?if($arResult["PROPERTIES"]["RADIUS"]["VALUE"][$key] == "Мото Диск"):?><span style="color:#000;font-size:12px;">за 2 диска:</span> <?=$priceX2?> руб.<?else:?><span style="color:#000;font-size:12px;">за 4 диска:</span> <?=$priceX4?> ₽<?endif;?></div>						
				<?endif;?><?endif;?><?endif;?>		
			<?endforeach;?>
...
<tr>
<td>
Введите диаметр дисков: </td>
<td><select data-cell="A1">
			<?foreach($arResult["PROPERTIES"]["PRICE"]["VALUE"] as $key => $arImage):?>
  ...
  <option value="<?=$priceX1?>"><?=$arResult["PROPERTIES"]["RADIUS"]["VALUE"][$key]?></option>
	<?endforeach;?>
</select>
</td>
</tr>
<tr>
<td>Количество дисков для покраски: </td>
<td><select data-cell="A2">
...
<input hidden data-cell="A5">
</td>
</tr>
<tr>
<td>
Количество дисков в ремонт: </td>
<td><select data-cell="A3">
...
<tr>
<td>
Требуется шиномонтаж?</td> <td><input id="CheckBox1" type="checkbox" class="CheckBoxClass" value="1" data-cell="A4"><label id="Label1" for="CheckBox1" class="CheckBoxLabelClass"></label></td>
<input hidden data-cell="A6">
<input hidden data-cell="A7">
<input hidden data-cell="A12" data-formula="A6*A2*A4">
<input hidden data-cell="A13" data-formula="A7*A2*A4">
</tr>
</table>
</div>
	<div class="col-2-div" style="line-height:2em;">
Ваш расчет:<br>
	Стоимость покраски <span class="rasch" data-cell="A10"></span><br>
	Стоимость ремонта <span class="rasch" data-cell="A11"></span><br>
		Стоимость шиномонтажа <span class="rasch" data-formula="A12"></span><sup>(легковой)</sup> | <span class="rasch" data-formula="A13"></span><sup>(кроссовер)</sup>
	<p></p>
		<b>Сумма Вашего заказа: <span class="rasch" data-formula="A10+A11+A12"></span><sup>(легковой)</sup> | <span class="rasch" data-formula="A10+A11+A13"></span><sup>(кроссовер)</sup></b>
</div>
</div>
	<p></p>
...
<tr style="outline: none medium;">
	<td style="outline: none medium;"><?$ppr=explode("-", $arResult["PROPERTIES"]["RADIUS"]["VALUE"][$key]);?>
		<span><?=$arResult["PROPERTIES"]["RADIUS"]["VALUE"][$key]?></span>
	</td>
    ...
    <?if($arResult["PROPERTIES"]["SHINOMO"]["VALUE"] == "Да"):?> 
		 <?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?><span class="oldprice"><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key]?> (1шт.) | <?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key] * 4?> комплект</span> <span class="newprice"><?=$priceX1?></span><?else:?><?=$priceX1?> (1шт.) | <?=$priceX1 * 4?> (4шт.)<?endif;?>
          <?else:?>
<?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?>
<div class="tabedis"><span class="litlet">Цена без скидки</span><br><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key]?></div>
...
<div class="tabedis"><span class="seldisrt">-<?=$arResult["PROPERTIES"]["AKCIYA3"]["VALUE"]?>%</span> <span class="litlet"><?=$arResult["PROPERTIES"]["NAKCIYA3"]["~VALUE"]?></span> <br><span class="newprice"><?=$priceX13?></span></div>
		...
		<?else:?><span ><input hidden value="<?=$priceX1?>" data-cell="M<?=$dq++;?>"><?=$priceX1?></span><?endif;?>
             <?endif;?>
	</td>
<?if($arResult["PROPERTIES"]["NEDISK"]["VALUE"] == "Да"):?>
<?else:?>
...
<td style="outline: none medium;">
 ...
 <?else:?>   
	<td style="outline: none medium;">
		<?if($arResult["PROPERTIES"]["MOTODISK"]["VALUE"] == "Да"):?>			
		<?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?>
		<div class="tabedis"><span class="litlet">Цена без скидки</span><br><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key] * 2?></div>
...
<?else:?>
		<?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?>
			<div class="tabedis"><span class="litlet">Цена без скидки</span><br><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key] * 4?></div>
...
<tr style="outline: none medium;">
	...
	<td style="outline: none medium;">
<?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?>
<div class="tabedis"><span class="litlet">Цена без скидки</span><br><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key1]+ $arResult["PROPERTIES"]["FIXSUM"]["VALUE"]?></div>
...
<?else:?>
	<td style="outline: none medium;">
				<?if($arResult["PROPERTIES"]["MOTODISK"]["VALUE"] == "Да"):?>			
		<?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?>
			<div class="tabedis"><span class="litlet">Цена без скидки</span><br><?=($arResult["PROPERTIES"]["PRICE"]["VALUE"][$key1]+$arResult["PROPERTIES"]["FIXSUM"]["VALUE"]) * 2?></div>
		...
		<?else:?>
		<?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?>
			<div class="tabedis"><span class="litlet">Цена без скидки</span><br><?=($arResult["PROPERTIES"]["PRICE"]["VALUE"][$key1]+$arResult["PROPERTIES"]["FIXSUM"]["VALUE"]) * 4?></div>
...
$( document ).ready( function ()
{
    ...
    autoCalculateTrigger : 'keyup',
		data: {
			A10: {formula: 'A1*A2'},
			A11: {formula: 'A5*A3'},
			A5: {formula: 'IF(A1=M1,1600,IF(A1=M2,1900,IF(A1=M3,2200,IF(A1=M4,2500,IF(A1=M5,2800,IF(A1=M6,3100,IF(A1=M7,3500,0)))))))'},
			A6: {formula: 'IF(A1=M1,760,IF(A1=M2,1000,IF(A1=M3,1120,IF(A1=M4,1320,IF(A1=M5,1480,IF(A1=M6,1680,IF(A1=M7,1800,0)))))))'},
			A7: {formula: 'IF(A1=M1,880,IF(A1=M2,1120,IF(A1=M3,1240,IF(A1=M4,1480,IF(A1=M5,1760,IF(A1=M6,2040,IF(A1=M7,2240,0)))))))'},
		}
});
	});
</script>
