...
<?endif;?><?endif;?><?endif;?>
 ...
 <span id="ot">От</span> <span id="odometer" class="odometer"><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][0] * $discount?></span> <i class="fa fa-rub"></i>
		...
		<?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?><span style="position: absolute;left: 35%;margin-top: -30px;text-decoration: line-through;color:red;">Старая цена: <span id="odometer2"><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][0]?></span> <i class="fa fa-rub"></i></span><?endif;?>
 <a class="ripplelink orange btn opener" style="line-height: 14px;" onclick="yaCounter16377226.reachGoal('ZAKAZ'); return true;">ЗАКАЗАТЬ</a> <a href="/akcii/" class="ripplelink orange btn" style="line-height: 14px;" onclick="yaCounter16377226.reachGoal('SKIDKA'); return true;">ХОЧУ СКИДКУ</a>
		...
		<div class="col-2-div uslugi" style="float:right;">
			...
			<?else:?>
			<div class="tabs_header2">выберите тип колпачка</div>
			<?endif;?>
			<div id="tabs">
				<ul>
			...
			<?foreach($arResult["PROPERTIES"]["PRICE"]["VALUE"] as $key => $arImage):?>
				...
				<?if($arResult["PROPERTIES"]["MOTODISK"]["VALUE"] == "Да"):?>			
				<div id="tabs-<?=$key?>"><span style="color:#000;">за 1 колпачок:</span> <?=$priceX1?> руб. <br /><span style="color:#000;">за 2 диска:</span> <?=$priceX2?> руб.</div>			
				<?else:?>
                ...
                <?else:?>
				<div id="tabs-<?=$key?>"><span style="color:#000;">за 1 колп.:</span> <?=$priceX1?> руб. <br /><?if($arResult["PROPERTIES"]["RADIUS"]["VALUE"][$key] == "Мото Диск"):?><span style="color:#000;">за 2 диска:</span> <?=$priceX2?> руб.<?else:?><span style="color:#000;">за 4 колп.:</span> <?=$priceX4?> руб.<?endif;?></div>						
				<?endif;?><?endif;?><?endif;?>		
			<?endforeach;?>
...
$('#ref-<?=$key?>').click(function(){
	...
	$('#odometer2').text('<?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key]?>');
setTimeout(function(){
...
<tr style="outline: none medium; background-color: #E4412F; color: #fff;" height="40px">
	<th style="outline: none medium;">
		 <?if($arResult["PROPERTIES"]["NEDISK"]["VALUE"] == "Да"):?>Тип<?else:?>Тип колпачка<?endif;?>
	</th>
	<th style="outline: none medium;">
...
<tr style="outline: none medium;">
    ...
    <?if($arResult["PROPERTIES"]["SHINOMO"]["VALUE"] == "Да"):?> 
		 <?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?><span class="oldprice"><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key]?> (1шт.) | <?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key] * 4?> комплект</span> <span class="newprice"><?=$priceX1?></span><?else:?><?=$priceX1?> (1шт.) | <?=$priceX1 * 4?> (4шт.)<?endif;?>
          <?else:?>
         	 <?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?><span class="oldprice"><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key]?></span> <span class="newprice"><?=$priceX1?></span><?else:?>от <?=$priceX1?><?endif;?>
             <?endif;?>
	</td>
<?if($arResult["PROPERTIES"]["NEDISK"]["VALUE"] == "Да"):?>
<?else:?>
...
<td style="outline: none medium;">
 ...
 <?else:?>   
	<td style="outline: none medium;">
				<?if($arResult["PROPERTIES"]["MOTODISK"]["VALUE"] == "Да"):?>			
				<?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?><span class="oldprice"><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key] * 2?></span> <span class="newprice"><?=$priceX2?></span><?else:?><?=$priceX2?><?endif;?>			
				<?else:?>
				<?if($arResult["PROPERTIES"]["RADIUS"]["VALUE"][$key] == "Мото Диск"):?><?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?><span class="oldprice"><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key] * 2?></span> <span class="newprice"><?=$priceX2?></span><?else:?><?=$priceX2?><?endif;?><?else:?><?if(!empty($arResult["PROPERTIES"]["AKCIYA"]["VALUE"])):?><span class="oldprice"><?=$arResult["PROPERTIES"]["PRICE"]["VALUE"][$key] * 4?></span> <span class="newprice"><?=$priceX4?></span><?else:?><?=$priceX4?><?endif;?><?endif;?>						
				<?endif;?>
	</td>
<?endif;?><?endif;?>
</tr>
