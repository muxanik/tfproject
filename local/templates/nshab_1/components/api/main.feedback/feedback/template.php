...
<div class="api-feedback <?=$tpl_class_name;?>" style="<?= $arParams['FORM_STYLE'] ?>">
	...
	<form action="<?=POST_FORM_ACTION_URI;?>" method="POST" enctype="multipart/form-data" name="api_feedback_form" id="<?= $arParams['UNIQUE_FORM_ID']; ?>">
		...
		<? if($arParams['USE_HIDDEN_PROTECTION']): ?>
			<input type="text" class="hidden_protection" name="HIDDEN[NAME]" value="<?=$arParams['HIDDEN']['NAME'];?>" />
			<input type="text" class="hidden_protection" name="HIDDEN[EMAIL]" value="<?=$arParams['HIDDEN']['EMAIL'];?>" />
			<input type="text" class="hidden_protection" name="hidden_protection" value="<?=$arParams['hidden_protection'];?>" />
		<? endif; ?>
